...
async function scrapeUtahBatch1() {
    console.log('🏆 Utah 2024-25 High School Boys - Batch 1 (Events 1-5)');
    console.log('📊 Processing first 5 events with COMPLETE data capture...');
    ...
    try {
        const results = await trackwrestlingScraperService.scrapeMatches({
            ...
            stateId: '50', // Utah
            headless: true,
            maxEvents: 5,  // Only 5 events per batch to ensure complete data
            maxTeams: null, // NO LIMIT - capture ALL teams and matches
            batchNumber: 1,
            batchDescription: 'Events 1-5 (Complete Data Capture)'
        });
        ...
        console.log('   node scripts/scrape-utah-batch-2.js (Events 6-10)');
...
// Run the scraper
scrapeUtahBatch1().then(() => {
    console.log('✅ Utah Batch 1 script completed');
    process.exit(0);
}).catch(error => {
