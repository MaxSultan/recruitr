...
async function scrapeUtahMicroBatch2() {
    console.log('🏆 Utah 2024-25 High School Boys - Micro Batch 2 (Events 3-4)');
    console.log('📊 Processing events 3-4 with COMPLETE data capture...');
    ...
    try {
        const results = await trackwrestlingScraperService.scrapeMatches({
            ...
            batchNumber: 2,
            batchDescription: 'Events 3-4 (Micro Batch - Complete Data Capture)'
        });

        console.log('\n🎉 Micro Batch 2 Completed!');
        ...
        console.log('   node scripts/scrape-utah-micro-batch-3.js (Events 5-6)');

    } catch (error) {
        console.error('❌ Micro Batch 2 failed:', error);
    }
}

// Run the scraper
scrapeUtahMicroBatch2().then(() => {
    console.log('✅ Utah Micro Batch 2 script completed');
    process.exit(0);
}).catch(error => {
